...
for i in $(seq 1 $N_NODES); do
    ...
    ./bftsmart server --id $i $VERBOSE_FLAG &
    PID=$!
    PIDS+=($PID)
    
    # Save the PID so we can target it later
    echo $PID > "/tmp/bft-smart$i.pid"
    echo "Started Node $i (PID: $PID) on localhost:$((8079 + i))"
done

echo "-------------------------------------------------------"
echo "Cluster is running."
echo "To kill node 2:   kill \$(cat /tmp/bft-smart2.pid)"
echo "To freeze node 2: kill -STOP \$(cat /tmp/bft-smart2.pid)"
echo "To resume node 2: kill -CONT \$(cat /tmp/bft-smart2.pid)"
